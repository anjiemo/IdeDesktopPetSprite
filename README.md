...
```bash
# Package the plugin zip (output at build/distributions/IdeDesktopPetSprite-1.0.6.zip)
./gradlew buildPlugin
```
...
**Settings → Plugins → ⚙ → Install Plugin from Disk… → choose `build/distributions/IdeDesktopPetSprite-1.0.6.zip`**, then restart.
