...
```bash
# 打包插件 zip（产物在 build/distributions/IdeDesktopPetSprite-1.0.4.zip）
./gradlew buildPlugin
```
...
**Settings → Plugins → ⚙ → Install Plugin from Disk… → 选择 `build/distributions/IdeDesktopPetSprite-1.0.4.zip`**，重启即可。
打开任意项目后桌面右下角即出现宠物。
